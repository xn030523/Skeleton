...
或用 `skeleton.yaml` / `~/.skeleton/config.yaml` 的 provider 形式：

```yaml
llm:
  provider: deepseek        # 自动解析 baseUrl、环境变量、默认模型
  model: deepseek-chat
fallback:
  provider: anthropic       # 主 provider 失败时回退
...
内置 MCP 宿主，50+ 精选服务器定义。默认全关，在 `skeleton.yaml` 或环境变量里单独开。
...
完整清单和每个服务器的配置见 [`skeleton.yaml`](skeleton.yaml)。OAuth 支持通过 `buildMcpOAuth()`。安装前 npm/pip 包恶意扫描用 `checkPackageForMalware()`。
...
| `SKELETON_API_KEY` | 是 | API 密钥 |
| `SKELETON_BASE_URL` | 是 | 基础 URL——`/v1` 之前 |
| `SKELETON_MODEL` | 是 | 模型名 |
...
在项目里建 `skeleton.yaml`，或 `~/.skeleton/config.yaml` 作为用户级默认：
...
compression:
  enabled: true
  threshold: 0.50        # 上下文用到 50% 时压缩
  targetRatio: 0.20      # 压缩到阈值的 20%
  protectLastN: 20       # 始终保留最近 N 条
...
值支持 `${VAR}` 替换。完整示例（含 34 个 provider 和 50+ MCP 服务器）见 [`config.example.yaml`](config.example.yaml)。
