/**
 * Skeleton Setup Wizard — provider-based interactive configuration.
 *
 * Layout (Hermes-style):
 *   ~/.skeleton/config.yaml  — settings (provider, model, mcp, etc.)
 *   ~/.skeleton/.env         — secrets (API keys)
 *
 * Instead of asking for protocol/apiKey/baseUrl/model separately,
 * the wizard asks for a provider name and auto-resolves everything.
 */
...
export async function runSetup(): Promise<void> {
  ...
  // Write API keys to ~/.skeleton/.env
  ...
  const yamlLines = [
    ...
    `# Secrets (API keys) are stored in ~/.skeleton/.env`,
  ...
  if (modelInput !== profile.defaultModel) {
    yamlLines.push(`  model: ${modelInput}`);
  }
  ...
  if (fallbackProfile) {
    ...
    if (fallbackModel !== fallbackProfile.defaultModel) {
      yamlLines.push(`  model: ${fallbackModel}`);
    }
  }

  if (mcpEnabled.length > 0) {
    yamlLines.push(``, `mcp:`, `  servers:`);
    for (const cat of mcpEnabled) {
      yamlLines.push(
        `    # ${MCP_CATEGORIES[cat] as string}: enabled`,
        `    ${cat}: { enabled: true }`,
      );
    }
  }
  ...
  if (write === "yes") {
    ...
    console.log(
      chalk.gray("\n  Skipped. You can create config.yaml manually."),
    );
  }
